...
angular.module("uiFormat").directive("uiNumber",function(){return{require:"ngModel",link:function(scope,element,attrs,ctrl){var _formatNumber=function(number){if(void 0===number&&(number=""),(number=number.replace(/[^0-9]+/g,"")).length>1&&(number=number.substring(0,1)+","+number.substring(1)),number.length>3){var length=(number=number.replace(/[^0-9]+/g,"")).length-2;number=number.substring(0,length)+","+number.substring(length)}return number};element.bind("keyup",function(){number=ctrl.$viewValue,number=_formatNumber(number),ctrl.$setViewValue(number),ctrl.$render()})}}});
